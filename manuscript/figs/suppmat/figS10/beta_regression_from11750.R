...
ordbetareg_plot <- ggplot(data = expected_predictions, aes(y=Estimate.migsorensen,x=clim_hpv_sorensen.mean, col = type, fill = type)) +
  ...
  annotate("rect", xmin = 0.297, xmax = 0.343, ymin = 0.00, ymax = 1, alpha = .7, fill = "white") + # white shade
  annotate("rect", xmin = 0.343, xmax = 0.351, ymin = 0.00, ymax = 1, alpha = 1, fill = "white") +
  ...
  # 
  # annotate("point", x=0.282, y = -0.032, shape = 24, size = 2, col = "black", fill = "#f69320") +  # ssp2 - 2060
  # annotate("text", label="SSP2", x=0.282, y = -0.087, size = 2.4, col = "#f69320", family= "Helvetica Narrow") +
  # 
  # annotate("point", x=0.341, y = -0.032, shape = 24, size = 2, col = "black", fill = "#bf1d1e") + # ssp5 - 2060
  # annotate("text", label="SSP5", x=0.341, y = -0.087, size = 2.4, col = "#bf1d1e", family= "Helvetica Narrow")
