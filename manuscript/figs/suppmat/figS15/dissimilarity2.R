...
dissimilarity <- ggplot() +
  ...
  geom_line(data = past_climdiss %>% dplyr::filter(clim_hpv_sorensen.year %in% (8000:8250)),
            aes(x = clim_hpv_sorensen.year, y = 1-clim_hpv_sorensen.mean), col = "#6867ac", linetype = "dotted", linesize = 0.4) +
  ...
  scale_y_continuous(expand = expansion(mult = c(0, 0)),
                     breaks = c(0.1, 0.6, round(1-past_climdiss[past_climdiss$clim_hpv_sorensen.year %in% c(seq(12000,11250,-250)), "clim_hpv_sorensen.mean"],2)),
                     ...
                     sec.axis = sec_axis(transform=~.*1, name="", breaks = c(0.1,0.6, round(1 - unlist(future_climdiss_ssp[future_climdiss_ssp$year == 2095, "median_ssp"]),2)), 
  ...
  geom_segment(aes(x = 7750, xend = 11550, 
  ...
  geom_segment(aes(x = 7750, xend = 11750, y= round(1 - unlist(future_climdiss_ssp[future_climdiss_ssp$year == 2095 & future_climdiss_ssp$scenario == "ssp585", 
                                                                                   "median_ssp"]),2), 
  ...
  ggstar::geom_star(aes(x = 7850, 
                        y = round(1 - unlist(future_climdiss_ssp[future_climdiss_ssp$year == 2095 & future_climdiss_ssp$scenario == "ssp245", 
                                                                 "median_ssp"]),2)), 
                    col = "grey20", fill = "#f69320", angle = 90, starshape = 11, size = 2.5) +
  ggstar::geom_star(aes(x = 7850, 
                        y = round(1 - unlist(future_climdiss_ssp[future_climdiss_ssp$year == 2095 & future_climdiss_ssp$scenario == "ssp585", 
                                                                 "median_ssp"]),2)), 
                    col = "grey20", fill = "#bf1d1e", angle = 90, starshape = 11, size = 2.5) +
  ...
  theme(panel.border = element_blank(), panel.grid.major = element_blank(),
        ...
        axis.title.y = element_text(colour = "black", family= "Helvetica Narrow", size = 8, margin = margin(r = 4.5), hjust = 0.95),
  ...
  geom_segment(aes(x = 7750, xend = 7750, y= 0.1, yend = 0.6), color = "black", linewidth = 0.5) +
  ...
  geom_text(aes(y = 0.05 + 0.03, x = 9500, label = "Sim. from 12k BP"), size = 7.5*5/14, family= "Helvetica Narrow") +
  ...
  geom_text(aes(y = 0.05 + 0.03 -0.1, x = 9500, label = "from 11.75k BP"), size = 7.5*5/14, family= "Helvetica Narrow") +
  ...
  geom_text(aes(y = 0.05 + 0.03 -0.2, x = 9500, label = "from 11.5k BP"), size = 7.5*5/14, family= "Helvetica Narrow") +
  ...
  geom_text(aes(y = 0.05 + 0.03 -0.3, x = 9500, label = "from 11.25k BP"), size = 7.5*5/14, family= "Helvetica Narrow") +
  ...
  geom_text(aes(y = 0.05 + 0.03 -0.4, x = 9500, label = "from 11k BP"), size = 7.5*5/14, family= "Helvetica Narrow") +
