...
if(start_year %in% c("12,000","11,750")){
  ...
  boxplot_performance <- ggplot(model_performance_withmig, aes(x = type, y = mig_sorensen, color = type, fill = type)) +
    ...
    theme(panel.border = element_blank(), panel.grid.major = element_blank(),
          ...
          plot.title = element_text(colour = "black", family= "Helvetica Narrow", size = 9),
          strip.background = element_blank(),
          strip.text = element_text(colour = "black", family= "Helvetica Narrow", size = 8)) +
    ...
    ggtitle(paste0("Migration from ",start_year," BP"))
