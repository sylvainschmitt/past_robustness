...
load_model_performance_withrep <- function(models, years, pollen_folder, nrep,
  ...
  model_performance <- lapply(1:nrow(models),function(i){
    ...
    perf <- lapply(1:nrep, function(r){
      
      perf <-lapply(years, function(year){
        cat(paste0(year, "\n"))
        
        distribution <- readRDS(file.path(mod$simfolder, paste0("rep", r, "_", year, "BP.rds")))
        ...
        return(data.frame(year = year, 
                          ...
                          mod = mod$name, rep = r, type = mod$type))})
