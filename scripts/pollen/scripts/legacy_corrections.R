...
ids_without_uncertainties <- unique(dataset_count[dataset_count$AgeModel_Source == "Neotoma" & dataset_count$`median_Age (cal. ka BP)` == 0 &
  ...
  left_join(fread(file.path(wd, "LegacyAge", "Table-S6_chronology_comparison.csv")), by = join_by(Dataset_ID))
...
chron_22636 <- data.table::fread(file.path("D:/species/pollen/herzschuhetal2021", "ages", "22636_17_ages.txt")) %>% # file send by Chenzhi Li
